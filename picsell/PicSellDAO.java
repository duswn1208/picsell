	...
	public List<PicSellDTO> getLists(int start, int end){
		...
		try {
			...
			sql = "select * from ("
					...
					+ "select uploadUserId,imagePoint,imageTitle,imageCode,fileName,originalName,imageTag,category "
...
public List<PicSellDTO> getReadMovie(String category) {
		...
		try {
			...
			sql = "select uploadUserId,imagePoint,imageTitle,imageCode,fileName,originalName,category from "
					+ "psimage where category='picture'";
		...
		return lists;
	}public List<PicSellDTO> getReadPicture(String category) {
