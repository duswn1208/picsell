...
public class PicsellImageDAO {
	...
	public List<PicsellImageDTO> famousList(){
		
		List<PicsellImageDTO> famous_list = new ArrayList<>();
		...
		try {
			...
			sql = "select * from ( select data.* from (select hitCount,imageCode,originalname,rank() over(order by hitCount desc) rank from psImage) data) where rank>=1 and rank<=4";
			...
			while (rs.next()) {
				...
				PicsellImageDTO dto = new PicsellImageDTO();
			...
			System.out.println(e.toString() + "picsellimageDao/famous_list");
		}
	...
	public List<PicBasketDTO> getLists(String userId) {
		...
		try {
			...
			while (rs.next()) {
				...
				bdto.setOriginalName(rs.getString("originalName"));
	...
	public int insertData(PicBasketDTO bdto) {
		...
		try {
			...
			sql = "insert into psBasket (num,downloadUser,imageCode,imagePoint,uploadUser,fileName,originalName) ";
			sql += "values(?,?,?,?,?,?,?)";
			...
			pstmt.setString(7, bdto.getOriginalName());
