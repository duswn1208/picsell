...
public class PicSellServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		PicSellPaymentDAO paymentDAO = new PicSellPaymentDAO(conn);
		PicsellUserDAO join_dao = new PicsellUserDAO(conn);
		PicsellImageDAO psImagedao = new PicsellImageDAO(conn);
		GuestDAO gDao = new GuestDAO(conn);
		PicSellBoardDAO board_dao = new PicSellBoardDAO(conn);

		MyUtil myUtil = new MyUtil();

		String root = getServletContext().getRealPath("/");
		String path = root + "pds" + File.separator + "saveImageFile";
		...
		if (uri.indexOf("charge.do") != -1) {
			...
			String sumPoint = req.getParameter("sumPoint");
			...
			if (userPoint < Integer.parseInt(sumPoint)) { // 다운로드 코인이 유저가 보유한 코인보다 클 경우
			 ...
			 * System.out.println("imagedto.getOriginalName() : " +
			 * imagedto.getOriginalName());
			 */

			// 이미지 무료
			System.out.println("path : " + path);
			boolean flag = FileManager.doZipFileDownload(resp, lists, path);
			...
			// 이미지 무료
			boolean flag = FileManager.doFileDownload(resp, imagedto.getOriginalName(), imagedto.getOriginalName(), path);
			System.out.println(flag);
		...
		if (uri.indexOf("main.do") != -1) {
			...
			String psUserId = req.getParameter("psUserId");

			String listUrl = cp + "/picsell/main.do";

			String deletePath = cp + "/picsell/delete.do";
			String imagePath = cp + "/pds/saveImageFile";
			String pageIndexList = myUtil.pageIndexList(currentPage, totalPage, listUrl);

			req.setAttribute("psUserId", psUserId);
			...
			if (dto.getImagePoint() == 0) {
				...
				// 이미지 무료
				boolean flag = FileManager.doFileDownload(resp, dto.getOriginalName(), dto.getOriginalName(), path);
			...
			bdto.setOriginalName(dto.getOriginalName());
		...
		} else if (uri.indexOf("list_picture.do") != -1) {
		...
		// -----------------------------
		...
		else if (uri.indexOf("down_x.do") != -1) {

			int num = Integer.parseInt(req.getParameter("num"));
			int result = gDao.deleteData(num);

			if (result != 1)
				System.out.println("down_x.do/삭제안됨");
			...
			// forward(req, resp, url);
			resp.sendRedirect(url);
		}else if (uri.indexOf("board_write.do") != -1) {
			...
			url = "/psPage/board_write.jsp";
			forward(req, resp, url);

		} else if (uri.indexOf("board_writeok.do") != -1) {
			PicSellBoardDTO board_dto = new PicSellBoardDTO();
			int maxNum = board_dao.getMaxNum();

			board_dto.setNum(maxNum + 1);
			board_dto.setSubject(req.getParameter("subject"));
			board_dto.setUserId(req.getParameter("userId"));
			board_dto.setContent(req.getParameter("content"));
			board_dto.setIpAddr(req.getRemoteAddr());

			board_dao.insertData(board_dto);

			url = cp + "/picsell/board_memory.do";
			resp.sendRedirect(url);

		} else if (uri.indexOf("board_memory.do") != -1) {
			// 넘어온 페이지 번호(myutil,article 수정,삭제)

			String pageNum = req.getParameter("pageNum");
			
			System.out.println(pageNum + "1111111111111111111111111111111111");
			...
			// 전체 데이터 구하기
			int dataCount = board_dao.getDataCount(searchKey, searchValue);
			...
			List<PicSellBoardDTO> board_lists = board_dao.getLists(start, end, searchKey, searchValue);
			String param = "";
			...
			// 페이지 처리
			String listUrl = cp + "/picsell/board_memory.do" + param;
			String pageIndexList // html 링크문장이 pageIndexList에들어감
					= myUtil.pageIndexList(currentPage, totalPage, listUrl);
			// article 주소정리 (사용자정의-하던말던내맘임)
			String articleUrl = cp + "/picsell/board_article.do";
			...
			board_dao.updateHitCount(num);

			PicSellBoardDTO board_dto = board_dao.getReadData(num);

			if (board_dto == null) {
				url = cp + "/picsell/board_memory.do";
				resp.sendRedirect(url);
			}
			int lineSu = board_dto.getContent().split("\n").length;
			board_dto.setContent(board_dto.getContent().replaceAll("\n", "<br/>"));
			...
			url = "/psPage/board_Article.jsp";
			forward(req, resp, url);
 
		} else if (uri.indexOf("board_updated.do") != -1) {
			
			PicSellCustomInfo info = (PicSellCustomInfo)req.getSession().getAttribute("PicSellCustomInfo");
			String userId =  info.getUserId();
			...
			PicSellBoardDTO board_dto = board_dao.getReadDatas(userId,num);
			...
			req.setAttribute("board_dto", board_dto);
			System.out.println(board_dto.getUserId());
			System.out.println(board_dto.getSubject());
			System.out.println(board_dto.getContent());
		req.setAttribute("pageNum", pageNum);
			req.setAttribute("params", param);
			req.setAttribute("searchKey", searchKey);
			req.setAttribute("searchValue", searchValue);
		
			url = "/psPage/board_Updated.jsp";
			forward(req, resp, url);

		}else if (uri.indexOf("board_updatedok.do") != -1) {
			...
			board_dto.setNum(num);
			board_dto.setSubject(req.getParameter("subject"));
			board_dto.setUserId(req.getParameter("userId"));
			board_dto.setContent(req.getParameter("content"));
			System.out.println(req.getParameter("subject")+req.getParameter("userId")+req.getParameter("content"));
			board_dao.updateData(board_dto);
			
			url = cp + "/picsell/board_memory.do?pageNum=" + pageNum + "&searchKey=" + searchKey + "&searchValue=" + searchValue;
			resp.sendRedirect(url); // 리다이렉트작업

		}else if (uri.indexOf("board_deleted_ok.do") != -1) {
			...
			url = cp + "/picsell/board_memory.do?" + params;
			resp.sendRedirect(url);
